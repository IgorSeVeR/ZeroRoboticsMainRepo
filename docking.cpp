...
void takeCube(int num)
{
    ...
    for(int i = 0; i<3; ++i)
        loc_tofly[i] = item_att[i]*dock_dis[num]/length + item_loc[i]; //i can tell u how it works
    ...
    api.setPositionTarget(loc_tofly);
    api.setAttitudeTarget(point_att);
    if ((mathVecMagnitude(myVel,3) < 0.01) && (getDis(myState,item_loc)<=dock_dis[num]) && (game.isFacingCorrectItemSide(num)) && (!(game.hasItem(num)))) game.dockItem(num);
}
